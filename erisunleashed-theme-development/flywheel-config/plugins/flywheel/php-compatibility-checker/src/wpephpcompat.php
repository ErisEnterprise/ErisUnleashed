...
class WPEPHPCompat {
	...
	public function start_test() {
		...
		*/
		...
		foreach ( $directories as $directory ) {
			...
			$scan_results .= __( 'Type', 'php-compatibility-checker' ) . ': ' . get_post_meta( $directory->ID, 'plugin_or_theme', true ) . "\n\n";
	...
	public function generate_directory_list() {
		...
		$plugin_base = $this->base . DIRECTORY_SEPARATOR;
		...
		foreach ( $all_plugins as $k => $v ) {
			...
			$id = $this->add_directory( $v['Name'], $plugin_path, 'plugin' );
		...
		foreach ( $all_themes as $k => $v ) {
			...
			$this->add_directory( $all_themes[ $k ]->Name, $theme_path, 'theme' );
		}
		...
		if ( 'yes' === $this->only_active && is_child_theme() ) {
			...
			$this->add_directory( $parent_theme_name, $parent_theme_path, 'theme' );
		}
	}
	 ...
	 * @return null
	 */
	private function add_directory( $name, $path, $plugin_or_theme ) {
		$dir = array(
			'post_title'   => $name,
			'post_content' => $path,
			'post_status'  => 'publish',
			...
			'post_type'    => 'wpephpcompat_jobs',
			'meta_input' => array(
				'plugin_or_theme' => $plugin_or_theme,
			)
		);
